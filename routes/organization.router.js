        ...
        if (connectionError) {
            ...
            client.query(orgQuery, [orgId], function (queryErr, orgResult) {
                ...
                if (queryErr) {
                    ...
                    var socialMediaQuery = "SELECT sm.id, sm.url, smt.id AS type_id, smt.name AS type, smt.logo FROM social_media sm " +
                    ...
                    client.query(socialMediaQuery, [orgId], function (queryErr, socialMediaResult) {
                        ...
                        if (queryErr) {
                            ...
                            var adsQuery = "SELECT ad.id, ad.name, ad.award, ad.advertiser, category.full_category as category, year.num AS year FROM ad " +
                            "INNER JOIN category ON ad.category_id = category.id " +
                            "INNER JOIN year ON ad.year_id = year.id " +
